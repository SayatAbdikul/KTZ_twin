    ...
    if speed_1s > 50.0 and hard_brake_command and brake_main_1s < 7.0:
        add_alert(
            _make_alert(
                ...
                "critical",
                "brakes",
                "Слабый отклик тормозов на скорости",
                "Тормозное усилие высокое, но отклик по давлению и замедление остаются недостаточными.",
                "Немедленно снизьте скорость и проверьте тормозные клапаны, цилиндры и пневмолинии.",
    ...
    if speed_1s > 60.0 and hard_brake_command and speed_drop_3s < 5.0:
        add_alert(
            _make_alert(
                ...
                "critical",
                "brakes",
                "Нет замедления при интенсивном торможении",
                "Скорость остаётся высокой несмотря на длительный сильный тормозной запрос.",
                "Переходите к процедуре экстренного торможения и проверьте тормозную систему перед продолжением движения.",
    ...
    if speed_1s > 50.0 and brake_pipe_1s < 4.3 and brake_main_1s < 7.0:
        add_alert(
            _make_alert(
                ...
                "critical",
                "pneumatic",
                "Потеря пневматического давления при торможении",
                "Давление в тормозной магистрали низкое, а пневморезерв не поддерживает требуемую остановку.",
                "Немедленно проверьте производительность компрессора, утечки и клапаны управления тормозами.",
    ...
    if oil_pressure_5s < oil_critical_low and speed_1s > 20.0:
        add_alert(
            _make_alert(
                ...
                "critical",
                "engine",
                "Критически низкое давление масла",
                "Давление смазки ниже безопасного рабочего диапазона при движении локомотива.",
                "Снимите нагрузку с двигателя и проверьте давление смазки перед продолжением эксплуатации.",
    ...
    if coolant_30s > coolant_critical_high or oil_temp_30s > oil_temp_critical_high or exhaust_30s > exhaust_critical_high:
        add_alert(
            _make_alert(
                ...
                "critical",
                "cooling",
                "Сильная тепловая перегрузка",
                "Рабочие температуры длительное время превышают критический диапазон.",
                "Снизьте тяговую нагрузку и проверьте контур охлаждения перед возвратом к полной мощности.",
    ...
    if speed_1s > 40.0 and brake_command and brake_main_1s < 7.2:
        add_alert(
            _make_alert(
                ...
                "warning",
                "brakes",
                "Обнаружено ухудшение торможения",
                "Тормозной запрос присутствует, но пневматический отклик ниже ожидаемого диапазона.",
                "Проверьте отклик по тормозному давлению и подготовьтесь к контролируемому снижению скорости.",
                ["pressure.brake_pipe", "pressure.brake_main"],
                now_ms,
            )
        )
    ...
    if high_current_low_accel:
        add_alert(
            _make_alert(
                ...
                "warning",
                "electrical",
                "Высокий ток при слабом ускорении",
                "Тяговый ток повышен, но отклик по скорости слабый, что указывает на повышенное сопротивление или плохое сцепление.",
                "Проверьте тяговое усилие, состояние контакта колесо-рельс и сопротивление привода.",
    ...
    if coolant_30s > coolant_warning_high:
        add_alert(
            _make_alert(
                ...
                "Температура охлаждения превысила предупредительный порог при длительной нагрузке.",
                "Снизьте нагрузку и проверьте поток воздуха и циркуляцию охлаждающей жидкости.",
    ...
    if oil_pressure_5s < oil_warning_low:
        add_alert(
            _make_alert(
                ...
                "Давление масла ниже ожидаемого рабочего диапазона.",
                "Проверьте давление смазки и избегайте длительной работы под высокой нагрузкой.",
                ["pressure.oil"],
                now_ms,
            )
        )
    ...
    if voltage_5s < voltage_warning_low and current_5s > 400.0:
        add_alert(
            _make_alert(
                ...
                "Тяговое напряжение падает, пока ток остаётся повышенным.",
                "Снизьте тяговую нагрузку и проверьте цепь электропитания.",
    ...
    if brake_pipe_1s < 4.8 and speed_1s > 20.0:
        add_alert(
            _make_alert(
                ...
                "info",
                "pneumatic",
                "Тенденция к снижению пневматического давления",
                "Давление в тормозной магистрали ниже номинального и требует наблюдения.",
                "Следите за стабильностью пневматического давления и подготовьте осмотр, если тенденция сохранится.",
                ["pressure.brake_pipe"],
                now_ms,
            )
        )
    ...
    if current_5s > current_warning_high:
        add_alert(
            _make_alert(
                ...
                "info",
                "electrical",
                "Повышенный тяговый ток",
                "Тяговый ток остаётся выше порога наблюдения.",
                "Следите за дополнительной нагрузкой, сцеплением и аномалиями напряжения.",
    ...
    if fuel_level_30s < fuel_warning_low:
        add_alert(
            _make_alert(
                ...
                "Запас топлива опустился ниже порога наблюдения.",
                "Запланируйте дозаправку и избегайте лишней работы под высокой нагрузкой.",
                ["fuel.level"],
                now_ms,
            )
        )
