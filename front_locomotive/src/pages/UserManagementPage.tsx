...
export function UserManagementPage() {
    ...
    const reloadThresholds = useCallback(async () => {
        ...
        try {
            ...
            setThresholdJson(JSON.stringify(config, null, 2))
        } catch (err) {
            setThresholdError(err instanceof Error ? err.message : 'Не удалось загрузить конфигурацию порогов.')
        } finally {
    ...
    async function handleSaveThresholds() {
        ...
        setThresholdSuccess(null)
        try {
            const parsed = JSON.parse(thresholdJson) as ThresholdConfig
            const saved = await saveThresholdConfig(accessToken, parsed)
            setThresholdJson(JSON.stringify(saved, null, 2))
            setThresholdSuccess('Конфигурация телеметрии успешно сохранена.')
        } catch (err) {
            if (err instanceof SyntaxError) {
                setThresholdError('Невалидный JSON. Проверьте синтаксис.')
            } else {
                setThresholdError(err instanceof Error ? err.message : 'Не удалось сохранить конфигурацию порогов.')
            }
        } finally {
        ...
        <div className="space-y-6 p-4 md:p-6">
            ...
            <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-5 shadow-2xl shadow-slate-950/25">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h2 className="text-lg font-semibold text-white">Конфигурация телеметрии</h2>
                        <p className="mt-1 text-sm text-slate-400">
                            Пороги, штрафы, статусы и edge-параметры. Изменения применяются без пересборки.
                        </p>
                    </div>
                    ...
                    >
                        {thresholdLoading ? 'Загрузка...' : 'Обновить'}
                    </button>
                </div>
                ...
                <div className="mt-4 space-y-3">
                    <textarea
                        value={thresholdJson}
                        onChange={(event) => setThresholdJson(event.target.value)}
                        className="min-h-[420px] w-full rounded-2xl border border-slate-700 bg-slate-900 px-4 py-3 font-mono text-xs leading-6 text-slate-100 outline-none transition-colors focus:border-blue-500"
                        spellCheck={false}
                        placeholder={`{\n  "metrics": {},\n  "penalties": {},\n  "healthStatus": {},\n  "edges": {}\n}`}
                    />
                    ...
                    <div className="flex flex-wrap gap-2">
                        <button
                            ...
                            disabled={thresholdSaving || thresholdLoading || !thresholdJson.trim()}
                            className="rounded-2xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-blue-500 disabled:cursor-not-allowed disabled:bg-slate-800 disabled:text-slate-500"
                        >
                            {thresholdSaving ? 'Сохранение...' : 'Сохранить конфигурацию'}
                        </button>
                    </div>
                </div>
            </section>
