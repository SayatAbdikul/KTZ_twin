...
function getMetric(
  ...
  return frame.readings.find((r) => r.metricId === metricId)?.value;
}
...
function computeHealth(
  ...
  let score = 100;
  if (fuel !== undefined && fuel < 25) score -= 20;
  if (fuel !== undefined && fuel < 12) score -= 20;
  if (coolant !== undefined && coolant > 95) score -= 20;
  if (coolant !== undefined && coolant > 105) score -= 20;
  if (current !== undefined && current > 1600) score -= 10;
  if (speed !== undefined && speed > 140) score -= 10;
...
export const useDispatcherStore = create<DispatcherState>((set) => ({
  ...
  setSelectedLocomotive: (selectedLocomotiveId) =>
    set({ selectedLocomotiveId }),
  ...
  upsertTelemetry: (frame) =>
    set((state) => {
      ...
      const updated: LocomotiveSnapshot = {
        ...
        speedKmh: speed ?? 0,
        fuelLevel: fuel ?? 0,
        coolantTemp: coolant ?? 0,
        tractionCurrent: current ?? 0,
        healthScore: score,
